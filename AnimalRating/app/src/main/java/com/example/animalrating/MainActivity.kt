...
class MainActivity : AppCompatActivity() {
    ...
    var itemClicked = 0 //This is used to pass the index of the selected picture to the next activity
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val imageIdList = listOf(
            R.id.dog_button, R.id.cat_button, R.id.bear_button,
            R.id.rabbit_button
        )//end list
        ...
        for ((index, imageId) in imageIdList.withIndex()) {
            findViewById<ImageButton>(imageId).setOnClickListener {
                when(index){
                    0 -> {
                        itemClicked = 0
                        animalIntent.putExtra("itemClicked", itemClicked)
